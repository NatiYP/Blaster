...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World) {
		...
		if (ConfirmHitResult.bBlockingHit) // we hit the head, return early
		{
			...
			return FServerSideRewindResult{ true, true }; // Confirmed a hit, Confirmed a head hit (headshot)
		}
			...
			if (ConfirmHitResult.bBlockingHit) { // got a hit, not a headshot
				ResetHitBoxes(HitCharacter, CurrentFrame);
				EnableCharacterMeshCollision(HitCharacter, ECollisionEnabled::QueryAndPhysics);
				return FServerSideRewindResult{ true, false }; // hit = true, headshot = false
			}
		}
	}
	// If we got here, the shot was a miss.
	ResetHitBoxes(HitCharacter, CurrentFrame);
	EnableCharacterMeshCollision(HitCharacter, ECollisionEnabled::QueryAndPhysics);
	return FServerSideRewindResult{ false, false }; // hit = false, headshot = false
}
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& HitLocation : HitLocations) {
		...
		if (World)
		{
			...
			if (BlasterCharacter) // got a valid hit. must be a HS because collision is enabled only for the head.
			{
				if (ShotgunResult.HeadShots.Contains(BlasterCharacter))
					ShotgunResult.HeadShots[BlasterCharacter]++;
				else ShotgunResult.HeadShots.Emplace(BlasterCharacter, 1); // first headshot, therefore 1
			}
		}
	}
	// enabled collision for all boxes, then disable for the head box
	for (auto& Frame : FramePackages) {
	...
	for (auto& HitLocation : HitLocations) {
		...
		if (World)
		{
			...
			if (BlasterCharacter) // got a valid hit. must be a HS because collision is enabled only for the head.
			{
				...
				else ShotgunResult.BodyShots.Emplace(BlasterCharacter, 1); // first bodyshot, therefore 1
			}
		}
	}
...
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (PathResult.HitResult.bBlockingHit) { // we hit the head, return early
		...
		UGameplayStatics::PredictProjectilePath(this, PathParams, PathResult);
		if (PathResult.HitResult.bBlockingHit) // Got a body hit.
		{
			ResetHitBoxes(HitCharacter, CurrentFrame);
			EnableCharacterMeshCollision(HitCharacter, ECollisionEnabled::QueryAndPhysics);
			return FServerSideRewindResult{ true, false }; // hit = true, headshot = false
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	for (auto& HitCharacter : HitCharacters) {
		if (nullptr == HitCharacter || nullptr == HitCharacter->GetEquippedWeapon() || nullptr == Character) continue;
...
void ULagCompensationComponent::SaveFramePackage()
{
	...
	if (FrameHistory.Num() <= 1) {
		...
		SaveFramePackage(ThisFrame); // Overload - NOT RECURSION
