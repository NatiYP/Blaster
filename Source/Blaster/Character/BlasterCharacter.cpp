...
ABlasterCharacter::ABlasterCharacter()
{
	...
	CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBook"));
	...
	LagCompensation = CreateDefaultSubobject<ULagCompensationComponent>(TEXT("LagCompensation")); // Used only on the server, so there's
	// no reason to replicate it.
	...
	GetCharacterMovement()->RotationRate = FRotator(0.f, 0.f, 670.f);
	...
	// Construct the hit boxes for server side rewind (Lag Compensation)
...
{
	if (nullptr == Combat || Combat->EquippedWeapon == nullptr) return;
...
void ABlasterCharacter::ReceiveDamage(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatorController, AActor* DamageCauser)
{
	...
	if (bElimmed || nullptr == BlasterGameMode) return;
...
void ABlasterCharacter::MulticastElim_Implementation(bool bPlayerLeftGame)
{
	...
	if (BlasterPlayerController) BlasterPlayerController->ClearHUDWeaponAmmo();
	bElimmed = true;
	PlayElimMontage();
