...
{
	if (HasAuthority())
		bShowTeamScores = bTeamsMatch; /* replicated because we want to show the teams GM hud to all the clients as well.
	(The server is setting it for himself here) */
	...
	if (BlasterHUD) {
		...
		if (bTeamsMatch) InitTeamScores(); else HideTeamScores();
	}
}
...
FString ABlasterPlayerController::GetInfoText(const TArray<class ABlasterPlayerState*>& Players)
{
	...
	if (Players.Num() == 0)
		InfoTextString = FString(Announcement::ThereIsNoWinner);
	else if (Players.Num() == 1 && Players[0] == BlasterPlayerState)
		InfoTextString = FString(Announcement::YouAreTheWinner);
...
void ABlasterPlayerController::ClientElimAnnouncement_Implementation(APlayerState* Attacker, APlayerState* Victim)
{
	...
	if (Attacker && Victim && Self)
	{
		...
		if (BlasterHUD)
		{
			...
			// General case. We were not involved in this kill.
...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime > CheckPingFrequency) {
		...
		if (PlayerState) {
			...
			if (PlayerState->GetPing() * 4 > HighPingThreshold) { // Multiply by 4 because ping is compressed to a fourth of our actual ping.
				HighPingWarning();
...
{
	if (bShowTeamScores) InitTeamScores(); else HideTeamScores();
}
...
void ABlasterPlayerController::SetHUDCarriedAmmo(int32 Ammo) {
	...
	if (BlasterHUD && BlasterHUD->CharacterOverlay && BlasterHUD->CharacterOverlay->CarriedAmmoAmount) {
		...
		if (BlasterHUD->CharacterOverlay->Slash) {
			FString SText = FString::Printf(TEXT("/"));
			BlasterHUD->CharacterOverlay->Slash->SetText(FText::FromString(SText));
		}
	}
	else {
...
void ABlasterPlayerController::ClearHUDWeaponAmmo()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	if (BlasterHUD && BlasterHUD->CharacterOverlay && BlasterHUD->CharacterOverlay->WeaponAmmoAmount) {
		...
		if (BlasterHUD->CharacterOverlay->CarriedAmmoAmount)
			BlasterHUD->CharacterOverlay->CarriedAmmoAmount->SetText(FText::FromString(AmmoText));
		if (BlasterHUD->CharacterOverlay->Slash)
			BlasterHUD->CharacterOverlay->Slash->SetText(FText::FromString(AmmoText));
	}
}
