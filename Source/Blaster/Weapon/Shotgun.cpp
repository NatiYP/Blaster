...
void AShotgun::FireShotgun(const TArray<FVector_NetQuantize>& HitTargets)
{
	...
	if (MuzzleFlashSocket) {
		...
		// Calculate body shot damage by multiplying times hit * body damage (and store in DamageMap)
		for (auto HitPair : HitMap) {
			if (HitPair.Key) {
		...
		// Calculate head shot damage by multiplying times hit * head damage (and store in DamageMap)
		...
		// Loop through DamageMap to apple the total damage for each character that was hit.
		// More efficient to apply damage once per character, instead of applying damage each pellet hit.
		for (auto DamagePair : DamageMap)
		{
			if (DamagePair.Key && InstigatorController)
			{
				...
				if (HasAuthority() && bCauseAuthDamage)
					UGameplayStatics::ApplyDamage(DamagePair.Key, // Character that was hit
						DamagePair.Value, // Total damage calculated in the two for loops above.
